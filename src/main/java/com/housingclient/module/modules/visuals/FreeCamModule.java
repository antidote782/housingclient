...
public class FreeCamModule extends Module {
    ...
    protected void onDisable() {
        ...
        // Restore player's yaw/pitch to the server-side values saved on enable
        if (mc.thePlayer != null) {
            ...
            mc.thePlayer.renderArmYaw = savedYaw;
            mc.thePlayer.prevRenderArmYaw = savedYaw;
            mc.thePlayer.renderArmPitch = savedPitch;
            mc.thePlayer.prevRenderArmPitch = savedPitch;
        }
    ...
    /**
     * Auto-disable on world unload to clean up ghost entities and prevent
     * stale nametags from persisting after switching worlds/servers.
     */
    @SubscribeEvent
    public void onWorldUnload(net.minecraftforge.event.world.WorldEvent.Unload event) {
        if (isEnabled()) {
            // Nullify entity refs since the world they belong to is being destroyed
            cameraEntity = null;
            ghostEntity = null;
            originalRenderEntity = null;
            setEnabled(false);
        }
    }
