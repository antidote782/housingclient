...
public class ConfigManager {
    ...
    public void loadConfig() {
        ...
        try (InputStreamReader reader = new InputStreamReader(new FileInputStream(configFile),
            ...
            // GUI settings
            if (json.has("guiKey"))
                guiKey = json.get("guiKey").getAsInt();
            if (json.has("rainbowMode"))
                rainbowMode = json.get("rainbowMode").getAsBoolean();
            if (json.has("rainbowSpeed"))
                rainbowSpeed = json.get("rainbowSpeed").getAsFloat();

            // Colors
            if (json.has("primaryColor"))
                primaryColor = parseColor(json.getAsJsonObject("primaryColor"));
            if (json.has("secondaryColor"))
                secondaryColor = parseColor(json.getAsJsonObject("secondaryColor"));
            if (json.has("backgroundColor"))
                backgroundColor = parseColor(json.getAsJsonObject("backgroundColor"));
            if (json.has("panelColor"))
                panelColor = parseColor(json.getAsJsonObject("panelColor"));

            // Bypass settings
            if (json.has("packetDelay"))
                packetDelay = json.get("packetDelay").getAsInt();
            if (json.has("randomizationFactor"))
                randomizationFactor = json.get("randomizationFactor").getAsDouble();
            if (json.has("humanizedMovement"))
                humanizedMovement = json.get("humanizedMovement").getAsBoolean();

            // HUD settings
            if (json.has("showModuleList"))
                showModuleList = json.get("showModuleList").getAsBoolean();
            if (json.has("showCoords"))
                showCoords = json.get("showCoords").getAsBoolean();
            if (json.has("showCPS"))
                showCPS = json.get("showCPS").getAsBoolean();
            if (json.has("showVisitorCount"))
                showVisitorCount = json.get("showVisitorCount").getAsBoolean();
            if (json.has("moduleListPosition"))
                moduleListPosition = json.get("moduleListPosition").getAsString();
    ...
    public void setRandomizationFactor(double factor) {
        this.randomizationFactor = factor;
    }

    public boolean isHumanizedMovement() {
        return humanizedMovement;
    }

    public void setHumanizedMovement(boolean humanized) {
        this.humanizedMovement = humanized;
    }
    ...
    public void setShowModuleList(boolean show) {
        this.showModuleList = show;
    }
    ...
    public boolean isShowVisitorCount() {
        return showVisitorCount;
    }

    public void setShowVisitorCount(boolean show) {
        this.showVisitorCount = show;
    }

    public String getModuleListPosition() {
        return moduleListPosition;
    }

    public void setModuleListPosition(String position) {
        this.moduleListPosition = position;
    }
}
